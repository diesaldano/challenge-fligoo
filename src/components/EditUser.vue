<template>
  <v-container align-center align-content-center >
    <v-layout align-center justify-center>
      <v-flex xs12 sm8 >
        <h1 class="title">Edit User</h1>
        <v-form @submit.prevent ref="form" lazy-validation v-model="valid">
          <v-text-field
            v-model="nameUser"
            type="text"
            placeholder="username"
            label="Username"
            :rules="rules.name"
          />
          <v-text-field
            v-model="last_name"
            type="text"
            label="Last Name"
            :rules="rules.last_name"
          /> 
          <v-text-field
            v-model="email"
            type="email"
            label="E-mail"
            :rules="rules.email"
          /> 
          <v-btn color="primary" @click="update">
              Update
          </v-btn>
        </v-form>
      </v-flex>
    </v-layout>
  </v-container>
</template>
<script>
export default {
    name: 'AddUser',
    created(){
      this.getUser()
    },
    computed: {
      nameUser:{
        get(){
          return this.user.first_name || ''
        },
        set(value){
          this.user.first_name = value
        }
      },
      last_name:{
        get(){
          return this.user.last_name || ''
        },
        set(value){
          this.user.last_name = value
        }
      },
      email:{
        get(){
          return this.user.email || ''
        },
        set(value){
          this.user.email = value
        }
      },
      avatar() {
        return `https://ui-avatars.com/api/?name=${this.first_name}+${this.last_name}&size=150`
      }
    },
    methods: {
      getUser(){
        let users = JSON.parse(localStorage.getItem('users'))
        let index = users.findIndex(user => user.id == this.$route.params.id)
        this.user = users[index]
      },
      update(){
        this.$refs.form.validate()
        if(this.valid){
          let user = {
            id: (this.$route.params.id == 'new')? Math.floor( Math.random() * 100 ) : this.user.id,
            email: this.email,
            first_name: this.nameUser,
            last_name: this.last_name,
            avatar: (this.$route.params.id == 'new')? this.avatar : this.user.avatar,
          }
          this.$emit('editUser', user)
          this.$router.push('/')
        }
      },
    },
    watch: {
    },
    data() {
      return {
        valid: false,
        user: null,
        rules: {
          email: [
            v => !!v || 'E-mail is required',
            v => /.+@.+\..+/.test(v) || 'E-mail must be valid'
          ],
          name: [
            (value) => !!value || 'Username is required',
            (value) => (value && value.length <= 10) || 'Username must be less than 10 characters',          
          ],
          last_name: [
            (value) => !!value || 'Last Name is required',
            (value) => (value && value.length <= 10) || 'Last Name must be less than 10 characters',          
          ],
        }
      }
    },
}
</script>